...
def download_acts(max_n_hot):
    ...
    acts_for_year_page_list = acts_by_year_page.get_for_year_page_list()
    n = 0
    n_hot = 0
    for acts_for_year_page in acts_for_year_page_list:
        act_metadata_list = acts_for_year_page.get_metadata_list()
        for act_metadata in act_metadata_list:
            ...
            logger(f'{emoji} {n_hot}/{max_n_hot}) {act_metadata.doc_num}')
            act_metadata.write()
            n += 1
            if is_hot:
                n_hot += 1
